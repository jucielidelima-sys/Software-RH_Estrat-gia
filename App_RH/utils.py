...
def criar_bases():
    ...
    return cargos, maturidade, skills, pdi
...
def garantir_estrutura():
    ...
    if not pdi_path.exists():
        _, _, _, pdi = criar_bases()
        pdi.to_excel(pdi_path, index=False)

def carregar_dados_iniciais():
    cargos, maturidade, skills, pdi = criar_bases()
    garantir_estrutura()
...
def salvar_pdi(df):
    garantir_estrutura()
    (Path("data") / "pdi_registros.xlsx").parent.mkdir(exist_ok=True)
    df.to_excel(Path("data") / "pdi_registros.xlsx", index=False)

def gerar_excel_export(cargos_df, maturidade_df, skills_df, pdi_df):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
...
def sidebar_global():
    ...
    if "dados_carregados" not in st.session_state:
        cargos_df, maturidade_df, skills_df, pdi_df = carregar_dados_iniciais()
        st.session_state["dados_carregados"] = {"cargos": cargos_df, "maturidade": maturidade_df, "skills": skills_df, "pdi": pdi_df}
    ...
    base_cargos, base_maturidade, base_skills, base_pdi = criar_bases()
    ...
    return cargos_df, maturidade_df, skills_df, pdi_df, filtro, cargo_selecionado
